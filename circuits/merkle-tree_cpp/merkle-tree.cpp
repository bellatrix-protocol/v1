...
void branch_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void branch_2_run(uint ctx_index,Circom_CalcWit* ctx);
void branch_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void branch_3_run(uint ctx_index,Circom_CalcWit* ctx);
void branch_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void branch_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
void merkleProof_7_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void merkleProof_7_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 5952;}

uint get_number_of_components() {return 30;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 5835;}
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[8]); // line circom 15
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[2])) + 0);
{
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[4]); // line circom 15
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[2])) + 0);
{
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[2]); // line circom 15
while(Fr_isTrue(&expaux[0])){
{
...
uint csoffset = mySignalStart+757;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "layers[0]";
mySubcomponents[aux_create+i] = aux_cmp_num;
branch_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 0 ;
aux_cmp_num += 0;
}
}
...
uint csoffset = mySignalStart+1499;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "layers[1]";
mySubcomponents[aux_create+i] = aux_cmp_num;
branch_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 0 ;
aux_cmp_num += 0;
}
}
...
uint csoffset = mySignalStart+2983;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "layers[2]";
mySubcomponents[aux_create+i] = aux_cmp_num;
branch_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 0 ;
aux_cmp_num += 0;
}
}
...
while(Fr_isTrue(&expaux[0])){
Fr_eq(&expaux[0],&lvar[1],&circuitConstants[4]); // line circom 36
if(Fr_isTrue(&expaux[0])){
{
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[3]].signalStart + 0],&circuitConstants[2]); // line circom 45
...
merkleProof_7_create(1,0,ctx,"main",0);
merkleProof_7_run(0,ctx);
}
